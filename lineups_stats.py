...
def lineups(goal_homepage):
    ...
    for link in lineups_link_lst:
        if str(m.team1).lower() in re.split("-|/", link) and str(m.team2).lower() in re.split("-|/", link):
            ...
            players_team2_str = '\n'.join(players_team2)
            api.update_status('Lineups ({}): \n{} \n\nLineups ({}): \n{}'.format(m.team1, players_team1_str, m.team2, players_team2_str))
...
def stats(goal_homepage):
    ...
    for link in stats_link_lst:
        if str(m.team1).lower() in re.split("-|/", link) and str(m.team2).lower() in re.split("-|/", link):
            ...
            api.update_status('Possession: {} \nShots on Target: {} \nTotal Passes: {}'.format(possession, shots_on_target, total_passes))


print(stats('https://www.goal.com/en-in/results/2021-06-12'))
